...
***Домашнее задание №13 Ansible: Разработка и тестирование Ansible ролей и плейбуков***

В ходе выполнения домашнего задания были установлены Virtualbox, Vagrant. Создан Vagrantfile, повторяющий нашу инфраструктуру в GCP и подняты 2 ВМ.
Добавлен провижининг для сервера dbserver, также создан плейбук base.yml для установки python, после этого была доработана роль db - добавлены таски установки и конфигурирования MongoDB. Аналогично была доработана роль app для нашего сервера приложений, заменили все упоминания appuser на переменную deploy_user.
Переопределили стандартное значение переменной пользователя на имя пользователя используемое нашим боксом:
...
В ходе выполнения дополнительного задания со (*) конфигурация Vagrant была дополнена для корректной работы проксирования приложения с помощью nginx:
...
```
Для тестирования ролей Ansible были установлены Molecule и Testinfra. Были прогнаны тесты для db роли, в том числе дополнительная - открыт ли порт БД.
Плейбуки для пакера сконфигурированы для использования ролей db и app, созданы новые образы.
